...
def main():
    ...
    try:
        while script_running:
            # Right-click at the beginning of the script
            pyautogui.rightClick()
            start_time = time.time()
            found_pos = find_first_hp_bar()
            elapsed_time = time.time() - start_time
            print(f"Spiral Search computing time: {elapsed_time:.2f} seconds, Clicking at {found_pos}...")
            # Make sure total charging time of skill is 5 seconds
            time.sleep(6.3 - elapsed_time)
